...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  if ( !is_initialized_) {
    ...
    // initializing finished, no predict or update needed
...
void UKF::Prediction(double delta_t) {
  ...
  MatrixXd Xsig_aug = GiveSigPts(x_aug, P_aug, lambda_, n_sig_);
  // 2. Predict Sigma Points.
  Xsig_pred_ = PredSigPts(Xsig_aug, delta_t, n_x_, n_sig_, std_a_, std_yawdd_);
 ...
 */
void UKF::NormAngOfComp(VectorXd vector, int index) {
 ...
 */
MatrixXd UKF::PredSigPts(MatrixXd Xsig, double delta_t, int n_x, int n_sig, double nu_am, double nu_yawdd) {
 ...
 *  @param n_sig: Sigma points dimension.
 */
MatrixXd UKF::GiveSigPts(VectorXd x, MatrixXd P, double lambda, int n_sig) {
  int n = x.size();
