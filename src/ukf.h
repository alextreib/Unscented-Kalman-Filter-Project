...
private:
   ...
   */
  MatrixXd GiveSigPts(VectorXd x, MatrixXd P, double lambda, int n_sig);
   ...
   */
  MatrixXd PredSigPts(MatrixXd Xsig, double delta_t, int n_x, int n_sig, double nu_am, double nu_yawdd);

  /**
   *  Normalized the component `index` of the vector `vector` to be inside [-M_PI, M_PI] interval.
   */
  void NormAngOfComp(VectorXd vector, int index);
public:
